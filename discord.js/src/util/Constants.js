 ...
 * 100% certain you don't need, as many are important, but not obviously so.
...
const Endpoints = exports.Endpoints = {
  User: userID => {
    ...
    return {
      ...
      Banner: (root, hash) => {
        if (userID === '1') return hash;
        return Endpoints.CDN(root).Banner(userID, hash);
      }
    };
  },
  guilds: '/guilds',
  ...
  CDN(root) {
    return {
      ...
      Banner: (userID, hash) => `${root}/banners/${userID}/${hash}.${hash.startsWith('a_') ? 'gif?size=1024' : 'png?size=2048'}`,
